...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        Retrieval-side scores are diagnostic only and stay out of this average.
...
def _clamp(value: float) -> float:
    """Keep a metric inside the [0.0, 1.0] range every RAGAS score lives in."""
    return max(0.0, min(1.0, value))


def _coverage(covering: set[str], reference: set[str]) -> float:
    """Fraction of ``reference`` tokens that ``covering`` also contains.

    An empty reference means "nothing to cover", which counts as full coverage.
    """
    if not reference:
        return 1.0
    return _clamp(len(covering & reference) / len(reference))
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial evaluator of a Student Services assistant.\n"
            "Score the answer on every rubric criterion using a 1-5 scale, where\n"
            "5 = correct, complete and well-grounded and 1 = wrong or irrelevant.\n"
            "Judge only factual grounding and rubric fit; never reward length,\n"
            "ordering, or a writing style that resembles your own.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer to score:\n{answer}\n\n"
            f"Rubric criteria:\n{criteria}\n\n"
            'Reply with JSON only, e.g. {"criterion": score, ...}, '
    ...
    def _parse_scores(raw: str, rubric: dict[str, Any]) -> dict[str, float]:
        """Extract criterion → score in [0, 1]; fall back to 0.5 on any doubt."""
        ...
        for name in rubric:
            ...
            if isinstance(value, bool) or not isinstance(value, (int, float)):
                scores[name] = 0.5
                continue
            # A judge may answer on the 1-5 rubric scale; normalise it to 0-1.
            normalised = value / 5.0 if value > 1.0 else float(value)
            scores[name] = _clamp(normalised)
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        def mean_score(entry: dict[str, Any]) -> float:
            values = [
                float(v)
                for v in (entry.get("scores") or {}).values()
                if isinstance(v, (int, float)) and not isinstance(v, bool)
            ]
            return sum(values) / len(values) if values else 0.0

        means = [mean_score(entry) for entry in scores_batch]
        if not means:
            return {
        ...
        # The batch is read as consecutive A/B comparisons: entry 0 vs entry 1,
        # entry 2 vs entry 3, ... Position bias shows up when the answer shown
        # first wins most of those pairs.
        pairs = list(zip(means[0::2], means[1::2]))
        first_wins = sum(1 for first, second in pairs if first > second)
        positional_bias = bool(pairs) and first_wins / len(pairs) > 0.6
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Keep the original golden record (with its metadata/id) attached.
            result.qa_pair = pair
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures):
            ...
            fix = (
                ...
                else "Review with the pipeline owner — no suggestion generated"
            )
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        playbook = {
            "hallucination": (
                "Add a grounding guardrail that rejects claims absent from the "
                "retrieved chunks, and cite the source document per claim"
            ),
            "irrelevant": (
                "Rewrite the prompt to restate the question and require every "
                "sub-question to be answered explicitly"
            ),
            "incomplete": (
                "Raise top_k and chunk size so multi-document policies are "
                "retrieved together, and few-shot the expected answer shape"
            ),
            "off_topic": (
                "Add intent detection and route out-of-scope questions to the "
                "scope policy in 00_system_scope.md"
            ),
            "refusal": (
                "Loosen over-strict guardrails so in-scope questions with "
                "sufficient evidence are answered instead of refused"
            ),
        }

        # Cluster first, fix the biggest cluster first (lecture: failure clustering).
        suggestions = [
            f"[{count}x {failure_type}] {playbook[failure_type]}"
        ...
        # Always leave the team at least three concrete, actionable steps.
        baseline = [
            "Add the failing questions to a regression suite that blocks deploy "
            "when any metric average drops by more than 0.05",
            "Rerank retrieved chunks by query overlap to lift Context Precision "
            "without changing the retrieved set",
            "Calibrate the LLM judge against 10 human-labelled cases before "
            "trusting its scores in CI",
        ]
